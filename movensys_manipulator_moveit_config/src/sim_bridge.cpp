...
private:
  ...
  std::vector<double> currentArmPositions() const;
  ...
  rclcpp::Publisher<sensor_msgs::msg::JointState>::SharedPtr pub_isaacsim_;
  rclcpp::Publisher<std_msgs::msg::Float64MultiArray>::SharedPtr pub_gazebo_;
...
: Node("sim_bridge")
{
  ...
  this->declare_parameter("gazebo_command_topic", "/gazebo_position_controller/commands");
  this->declare_parameter("isaacsim_command_topic", "/joint_command");
  ...
  const auto gazebo_command_topic = this->get_parameter("gazebo_command_topic").as_string();
  const auto isaacsim_command_topic = this->get_parameter("isaacsim_command_topic").as_string();
  ...
  gripper_state_ = gripper_open_;

  pub_isaacsim_ = this->create_publisher<sensor_msgs::msg::JointState>(
    isaacsim_command_topic, 10);
  pub_gazebo_ = this->create_publisher<std_msgs::msg::Float64MultiArray>(
    gazebo_command_topic, 10);

  sub_joint_state_ = this->create_subscription<sensor_msgs::msg::JointState>(
    joint_states_topic, 10,
    std::bind(&SimBridge::cbJointStates, this, std::placeholders::_1));

  set_gripper_srv_ = this->create_service<std_srvs::srv::SetBool>(
    set_gripper_service,
    std::bind(
      &SimBridge::setGripper, this,
  ...
  action_server_ = rclcpp_action::create_server<FollowJT>(
    ...
    action_name,
    std::bind(
      &SimBridge::handle_goal, this,
      std::placeholders::_1, std::placeholders::_2),
    std::bind(&SimBridge::handle_cancel, this, std::placeholders::_1),
    std::bind(&SimBridge::handle_accepted, this, std::placeholders::_1));

  sub_servo_ = this->create_subscription<trajectory_msgs::msg::JointTrajectory>(
    servo_command_topic, 10,
    std::bind(&SimBridge::cbServoCommand, this, std::placeholders::_1));
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "sim_bridge is ready (arm joints: %zu, gripper joints: %zu)",
    joint_names_.size(), gripper_joint_names_.size());
  RCLCPP_INFO(
    this->get_logger(), "Commanding Isaac Sim on '%s' and Gazebo on '%s'",
    isaacsim_command_topic.c_str(), gazebo_command_topic.c_str());
}

void SimBridge::cbJointStates(const sensor_msgs::msg::JointState::SharedPtr msg)
{
  last_joint_state_ = *msg;
}

std::vector<double> SimBridge::currentArmPositions() const
{
  std::vector<double> positions(joint_names_.size(), 0.0);

  for (size_t i = 0; i < joint_names_.size(); ++i) {
    auto it = std::find(
      last_joint_state_.name.begin(), last_joint_state_.name.end(), joint_names_[i]);
    ...
    if (idx < last_joint_state_.position.size()) {
      positions[i] = last_joint_state_.position[idx];
    }
  }
...
void SimBridge::publishJointCommand(
  const std::vector<double> & arm_positions,
  const std::vector<double> & arm_velocities)
{
  const size_t n_arm = joint_names_.size();
  const size_t n_total = n_arm + gripper_joint_names_.size();

  std::vector<double> positions = currentArmPositions();
  std::copy_n(arm_positions.begin(), std::min(arm_positions.size(), n_arm), positions.begin());
  if (arm_positions.size() < n_arm) {
    RCLCPP_WARN_ONCE(
      this->get_logger(),
      "Command carries %zu of %zu arm joints; holding the rest at their current pose",
      arm_positions.size(), n_arm);
  }

  std::vector<double> velocities(n_arm, 0.0);
  std::copy_n(arm_velocities.begin(), std::min(arm_velocities.size(), n_arm), velocities.begin());

  positions.resize(n_total, gripper_state_);
  velocities.resize(n_total, 0.0);

  sensor_msgs::msg::JointState joint_command;
  joint_command.name = joint_names_;
  joint_command.name.insert(
    joint_command.name.end(),
    gripper_joint_names_.begin(), gripper_joint_names_.end());
  joint_command.position = positions;
  joint_command.velocity = velocities;
  joint_command.header.stamp = this->get_clock()->now();
  pub_isaacsim_->publish(joint_command);

  std_msgs::msg::Float64MultiArray motor_data;
  motor_data.data.assign(positions.begin(), positions.end());
  pub_gazebo_->publish(motor_data);
}
...
void SimBridge::publishExecActive(bool active)
{
  ...
  std::shared_ptr<std_srvs::srv::SetBool::Response> response)
{
  ...
  const auto positions = currentArmPositions();
  publishJointCommand(positions, std::vector<double>(positions.size(), 0.0));

  RCLCPP_INFO(this->get_logger(), "Gripper: %s", request->data ? "close" : "open");
}

rclcpp_action::GoalResponse SimBridge::handle_goal(
  const rclcpp_action::GoalUUID &,
  std::shared_ptr<const FollowJT::Goal> goal)
{
  (void)goal;
...
rclcpp_action::CancelResponse SimBridge::handle_cancel(
  const std::shared_ptr<GoalHandleFJT> /*goal_handle*/)
{
  return rclcpp_action::CancelResponse::ACCEPT;
}

void SimBridge::handle_accepted(const std::shared_ptr<GoalHandleFJT> goal_handle)
{
  std::thread(&SimBridge::execute, this, goal_handle).detach();
}
